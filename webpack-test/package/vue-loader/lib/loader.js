...
module.exports = function (content) {
  ...
  // parse will use template-compiler
  const parts = parse(
    content,
    fileName,
    this.sourceMap,
    sourceRoot,
    false // needCssSourceMap
  )
  // parts is a SFCDescriptor 
  // struct {
  //   template: null,
  //   script: null,
  //   styles: [],
  ...
  console.log('SFCDescriptor: ', parts)

  // 判断 styles 数组中是否有存在 scoped style 的
  ...
  if (parts.styles.length) {
    ...
    let styleRequire = 'var injectStyle = '
    // if (needsHotReload) {            // hot reload
    //   styleInjectionCode += `  if (disposed) return\n`
    // }
    // if (needsExplicitInjection) {    // server or shadowMode
    ...
    parts.styles.forEach((style, i) => {
      ...
      console.log('styles requireString: ', requireString)
