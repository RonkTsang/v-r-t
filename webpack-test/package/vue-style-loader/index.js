...
module.exports = function (css) {
  _c.log('vue-style-loader test', css)
}

/* module.exports.pitch = function (remainingRequest) {
  _c.log('vue-style-loader remainingRequest', remainingRequest)
  ...
  if (options.shadowMode) {
    ...
    let res = shared.concat(code).join('\n')
    _c.log('vue-style-loader res', res)
    return res
  } else {
    ...
    if (isVue) {
      ...
      // ]).join('\n')
      let res = shared.concat([
        'require(' + addStylesServerPath + ').default(' + id + ', content, ' + isProduction + ')'
      ]).join('\n')

      // _c('vue-style-loader res', res)
